...
import { listProducts } from "~/lib/sync/api/products.server";

// Read-only browser for the synced "products" table. Pagination + search
...
export async function loader({ context, request }: LoaderFunctionArgs) {
  ...
  const result = await listProducts(env.D1, {
    shop,
    limit: PAGE_SIZE,
...
export default function ProductsDataRoute() {
  ...
  return (
    <main style={{ fontFamily: "system-ui", padding: "2rem", maxWidth: 1200 }}>
      ...
      <h1 style={{ marginTop: "1rem" }}>Products</h1>
          ...
          <tbody>
            {data.rows.map((row) => (
              <tr key={row.remoteId} style={{ borderBottom: "1px solid #eee" }}>
                ...
                <td className="px-3 py-2 font-mono text-xs">{formatCell(row.title)}</td>
                <td className="px-3 py-2 font-mono text-xs">{formatCell(row.handle)}</td>
                <td className="px-3 py-2 font-mono text-xs">{formatCell(row.status)}</td>

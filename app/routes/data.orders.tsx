...
import { listOrders } from "~/lib/sync/api/orders.server";

// Read-only browser for the synced "orders" table. Pagination + search
...
export async function loader({ context, request }: LoaderFunctionArgs) {
  ...
  const result = await listOrders(env.D1, {
    shop,
    limit: PAGE_SIZE,
...
export default function OrdersDataRoute() {
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.875rem" }}>
          <thead style={{ background: "#f5f5f5" }}>
            <tr>
              ...
              <th className="px-3 py-2 text-left">totalPrice</th>
              <th className="px-3 py-2 text-left">financialStatus</th>
          ...
          <tbody>
            {data.rows.map((row) => (
              <tr key={row.remoteId} style={{ borderBottom: "1px solid #eee" }}>
                ...
                <td className="px-3 py-2 font-mono text-xs">{formatCell(row.name)}</td>
                <td className="px-3 py-2 font-mono text-xs">{formatCell(row.email)}</td>
                <td className="px-3 py-2 font-mono text-xs">{formatCell(row.totalPrice)}</td>
                <td className="px-3 py-2 font-mono text-xs">{formatCell(row.financialStatus)}</td>
